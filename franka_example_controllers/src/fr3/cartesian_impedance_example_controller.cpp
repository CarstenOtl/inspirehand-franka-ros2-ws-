...
CartesianImpedanceExampleController::CartesianGains CartesianImpedanceExampleController::buildGains(
  ...
  CartesianGains gains;
  for (int i = 0; i < num_cartesian_dof; ++i) {
    const double k_i = std::max(0.0, k[i]);
    gains.stiffness(i, i) = k_i;
    gains.damping(i, i) = 2.0 * std::sqrt(k_i);  // critically-damped
  }
  return gains;
}
...
CartesianImpedanceExampleController::command_interface_configuration() const {
  ...
  for (int i = 1; i <= num_joints; ++i) {
    command_interfaces_config.names.push_back(arm_prefix_ + robot_type_ + "_joint" +
...
CartesianImpedanceExampleController::state_interface_configuration() const {
  ...
  for (int i = 1; i <= num_joints; ++i) {
    state_interfaces_config.names.push_back(arm_prefix_ + robot_type_ + "_joint" +
...
CartesianImpedanceExampleController::on_configure(
  ...
  const double rotational_stiffness = get_node()->get_parameter("rotational_stiffness").as_double();
  const double nullspace_stiffness = get_node()->get_parameter("nullspace_stiffness").as_double();

  CartesianGains gains =
      buildGains({translational_stiffness, translational_stiffness, translational_stiffness,
  ...
  nullspace_stiffness_buffer_.initRT(nullspace_stiffness);
...
CartesianImpedanceExampleController::CallbackReturn
CartesianImpedanceExampleController::on_deactivate(
    const rclcpp_lifecycle::State& /*previous_state*/) {
...
rcl_interfaces::msg::SetParametersResult CartesianImpedanceExampleController::onParameterUpdate(
  ...
  for (const auto& parameter : parameters) {
    if (parameter.get_name() == "nullspace_stiffness") {
      ...
      nullspace_stiffness_buffer_.writeFromNonRT(value);
    }
  }
  return result;
}
