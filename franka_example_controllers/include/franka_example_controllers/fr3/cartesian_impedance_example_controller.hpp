...
/**
 * Cartesian impedance controller. A spring/damper between the measured end-effector pose and an
 * equilibrium pose, mapped to joint torques through the transposed Jacobian, with a nullspace
 * term that pulls the arm back to the configuration it was activated in.
 *
 * The equilibrium pose follows an internal periodic demo motion until something publishes on
 * ``~/equilibrium_pose``; Cartesian stiffness can be changed at runtime through
 * ``~/set_cartesian_stiffness`` and the nullspace stiffness through the parameter of the same
 * name. All three cross into the update loop through realtime buffers.
 */
