...
class ReplayClient(Node):
    def __init__(self, config, node_name='trajectory_replay_client'):
    ...
    def _wait_command(self, active_before, rejections_before, timeout, description, accept_timeout=5.0):
        def accepted():
            status = self.status()
